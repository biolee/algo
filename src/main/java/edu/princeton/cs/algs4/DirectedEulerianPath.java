...
/**
 * The <tt>DirectedEulerianPath</tt> class represents a data type for finding an Eulerian path in a
 * digraph. An <em>Eulerian path</em> is a path (not necessarily simple) that uses every edge in the
 * digraph exactly once.
 *
 * <p>This implementation uses a nonrecursive depth-first algo.search. The constructor runs in O(E +
 * V) time, and uses O(V) extra space, where E is the number of edges and V the number of vertices
...
public class DirectedEulerianPath {
  ...
  public DirectedEulerianPath(Digraph G) {
    ...
    // greedily add to cycle, depth-first algo.search style
   ...
   *
   * @return the sequence of vertices on an Eulerian path; <tt>null</tt> if no such path
   */
   ...
   *
   * @return <tt>true</tt> if the digraph has an Eulerian path; <tt>false</tt> otherwise
   */
