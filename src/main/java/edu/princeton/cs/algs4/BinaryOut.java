...
/**
 * <i>Binary output</i>. This class provides methods for converting primtive type variables
 * (<tt>boolean</tt>, <tt>byte</tt>, <tt>char</tt>, <tt>int</tt>, <tt>long</tt>, <tt>float</tt>, and
 * <tt>double</tt>) to sequences of bits and writing them to an output stream. The output stream can
 * be standard output, a file, an OutputStream or a Socket. Uses big-endian (most-significant byte
 * first).
 *
 * <p>The client must <tt>flush()</tt> the output stream when finished writing bits.
 *
 * <p>The client should not intermixing calls to <tt>BinaryOut</tt> with calls to <tt>Out</tt>;
