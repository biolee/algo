...
/**
 * The <tt>DigraphGenerator</tt> class provides static methods for creating various digraphs,
 ...
 *
 * <p>For additional documentation, see <a href="http://algs4.cs.princeton.edu/42digraph">Section
...
public class DigraphGenerator {
   ...
   * model. This implementations takes time propotional to V^2 (even if <tt>p</tt> is small).
   *
   * @param V the number of vertices
  ...
  /**
   * Returns a random tournament digraph on <tt>V</tt> vertices. A tournament digraph is a DAG in
  ...
  /**
   * Returns a random rooted-in DAG on <tt>V</tt> vertices and <tt>E</tt> edges. A rooted in-tree is
   * a DAG in which there is a single vertex reachable from every other vertex. The DAG returned is
  ...
  /**
   * Returns a random rooted-out DAG on <tt>V</tt> vertices and <tt>E</tt> edges. A rooted out-tree
   * is a DAG in which every vertex is reachable from a single vertex. The DAG returned is not
   * chosen uniformly at random among all such DAGs.
   *
   * @param V the number of vertices
   * @param E the number of edges
  ...
  /**
   * Returns a random rooted-in tree on <tt>V</tt> vertices. A rooted in-tree is an oriented tree in
  ...
  /**
   * Returns a random rooted-out tree on <tt>V</tt> vertices. A rooted out-tree is an oriented tree
   ...
   * @return a digraph that is a directed Eulerian cycle on <tt>V</tt> vertices and <tt>E</tt> edges
   ...
   * @return a digraph that is a directed Eulerian path on <tt>V</tt> vertices and <tt>E</tt> edges
  ...
  /**
   * Returns a random simple digraph on <tt>V</tt> vertices, <tt>E</tt> edges and (at least)
   * <tt>c</tt> strong components. The vertices are randomly assigned integer labels between
   * <tt>0</tt> and <tt>c-1</tt> (corresponding to strong components). Then, a strong component is
   * creates among the vertices with the same label. Next, random edges (either between two vertices
   * with the same labels or from a vetex with a smaller label to a vertex with a larger label). The
   * number of components will be equal to the number of distinct labels that are assigned to
   * vertices.
   *
   * @param V the number of vertices
   * @param E the number of edges
   * @param c the (maximum) number of strong components
   * @return a random simple digraph on <tt>V</tt> vertices and <tt>E</tt> edges, with (at most)
   *     <tt>c</tt> strong components
