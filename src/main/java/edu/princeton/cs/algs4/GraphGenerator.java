...
/**
 * The <tt>GraphGenerator</tt> class provides static methods for creating various graphs, including
 * Erdos-Renyi random graphs, random bipartite graphs, random k-regular graphs, and random rooted
 * trees.
 *
 * <p>For additional documentation, see <a href="http://algs4.cs.princeton.edu/41graph">Section
...
public class GraphGenerator {
  ...
  /**
   * Returns a random simple graph on <tt>V</tt> vertices, with an edge between any two vertices
   * with probability <tt>p</tt>. This is sometimes referred to as the Erdos-Renyi random graph
   * model.
   *
   * @param V the number of vertices
   * @param p the probability of choosing an edge
   * @return a random simple graph on <tt>V</tt> vertices, with an edge between any two vertices
  ...
  /**
   * Returns a random simple bipartite graph on <tt>V1</tt> and <tt>V2</tt> vertices, containing
   * each possible edge with probability <tt>p</tt>.
   *
   * @param V1 the number of vertices in one partition
   * @param V2 the number of vertices in the other partition
   * @param p the probability that the graph contains an edge with one endpoint in either side
   * @return a random simple bipartite graph on <tt>V1</tt> and <tt>V2</tt> vertices, containing
   *     each possible edge with probability <tt>p</tt>
   * @throws IllegalArgumentException if probability is not between 0 and 1
   */
   ...
   * @return a graph that is an Eulerian cycle on <tt>V</tt> vertices and <tt>E</tt> edges
   ...
   * @return a graph that is an Eulerian path on <tt>V</tt> vertices and <tt>E</tt> edges
   ...
   * @return a wheel graph on <tt>V</tt> vertices: a single vertex connected to every vertex in a
   *     cycle on <tt>V-1</tt> vertices
   */
   ...
   * @return a star graph on <tt>V</tt> vertices: a single vertex connected to every other vertex
   */
  ...
  /**
   * Returns a uniformly random <tt>k</tt>-regular graph on <tt>V</tt> vertices (not necessarily
