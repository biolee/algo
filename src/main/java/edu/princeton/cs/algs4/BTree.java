...
/**
 * The <tt>BTree</tt> class represents an ordered symbol table of generic key-value pairs. It
 ...
 * cannot be <tt>null</tt>&mdash;setting the value associated with a key to <tt>null</tt> is
 * equivalent to deleting the key from the symbol table.
 *
 * <p>This implementation uses a B-tree. It requires that the key type implements the
 * <tt>Comparable</tt> interface and calls the <tt>compareTo()</tt> and method to compare two keys.
 * It does not call either <tt>equals()</tt> or <tt>hashCode()</tt>. The <em>get</em>, <em>put</em>,
 * and <em>contains</em> operations each make log<sub><em>m</em></sub>(<em>n</em>) probes in the
 * worst case, where <em>n</em> is the number of key-value pairs and <em>m</em> is the branching
 * factor. The <em>size</em>, and <em>is-empty</em> operations take constant time. Construction
 * takes constant time.
 *
 * <p>For additional documentation, see <a href="http://algs4.cs.princeton.edu/62btree">Section
