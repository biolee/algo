...
/**
 * The <tt>FloydWarshall</tt> class represents a data type for solving the all-pairs shortest paths
 ...
 * Afterwards, the <tt>dist()</tt>, <tt>hasPath()</tt>, and <tt>hasNegativeCycle()</tt> methods take
 * constant time; the <tt>path()</tt> and <tt>negativeCycle()</tt> method takes time proportional to
...
public class FloydWarshall {
   ...
   * digraph <tt>G</tt>. If no such shortest path exists for some pair of vertices, it computes a
   * negative cycle.
   *
   ...
   *
   * @return a negative cycle as an iterable of edges, or <tt>null</tt> if there is no such cycle
   */
   ...
   * @return <tt>true</tt> if there is a path from vertex <tt>s</tt> to vertex <tt>t</tt>, and
   ...
   * @param s the source vertex
   * @param t the destination vertex
   * @return a shortest path from vertex <tt>s</tt> to vertex <tt>t</tt> as an iterable of edges,
   *     and <tt>null</tt> if no such path
   * @throws UnsupportedOperationException if there is a negative cost cycle
   */
