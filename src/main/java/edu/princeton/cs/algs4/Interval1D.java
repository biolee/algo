...
/**
 * The <tt>Interval1D</tt> class represents a one-dimensional interval. The interval is
 * <em>closed</em>&mdash;it contains both endpoints. Intervals are immutable: their values cannot be
 * changed after they are created. The class <code>Interval1D</code> includes methods for checking
...
public class Interval1D {
   ...
   * @throws IllegalArgumentException if either <tt>min</tt> or <tt>max</tt> is <tt>Double.NaN</tt>,
   ...
   * @return <tt>true</tt> if this interval equals the other interval; <tt>false</tt> otherwise
   */
