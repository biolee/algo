...
/**
 * The <code>PatriciaSET</code> class provides an implementation of an unordered set, with the
 * restriction that the items (keys) are of class {@link String}. It supports the usual
 ...
 * class can only accommodate keys of class {@link String}. This unfortunate restriction stems from
 * a limitation in Java. Although Java provides excellent support for generic programming, the
 * current infrastructure somewhat limits generic collection implementations to those that employ
 * comparison-based or hash-based methods. PATRICIA does not employ comparisons or hashing; instead,
 * it relies on bit-test operations. Because Java does not furnish any generic abstractions (or
 * implementations) for bit-testing the contents of an object, providing support for generic keys
 * using PATRICIA does not seem practical.
 *
...
public class PatriciaSET implements Iterable<String> {
  ...
  /**
   * Unit tests the <code>PatriciaSET</code> data type. This test fixture runs a series of tests on
   * a randomly generated dataset. You may specify up to two integer parameters on the command line.
   ...
   *
   * @param key the key
   * @return <code>true</code> if the set contains <code>key</code> and <code>false</code> otherwise
   * @throws NullPointerException if <code>key</code> is <code>null</code>
   * @throws IllegalArgumentException if <code>key</code> is the empty string.
   */
   ...
   * named <code>set</code>, use the foreach notation: <code>for (Key key : set)</code>.
   *
