 ...
 * files: http://algs4.cs.princeton.edu/31elementary/tinyST.txt
 *
 * <p>Symbol table implementation with binary algo.search in an ordered array.
 *
 ...
 * java.util.Map}, this class uses the convention that values cannot be <tt>null</tt>&mdash;setting
 * the value associated with a key to <tt>null</tt> is equivalent to deleting the key from the
 * symbol table.
 *
 * <p>This implementation uses a sorted array. It requires that the key type implements the
 * <tt>Comparable</tt> interface and calls the <tt>compareTo()</tt> and method to compare two keys.
 * It does not call either <tt>equals()</tt> or <tt>hashCode()</tt>. The <em>put/em> and
 * <em>remove</em> operations each take linear time in the worst case; the <em>contains</em>,
 * <em>ceiling</em>, <em>floor</em>, and <em>rank</em> operations take logarithmic time; the
 * <em>size</em>, <em>is-empty</em>, <em>minimum</em>, <em>maximum</em>, and <em>select</em>
 * operations take constant time. Construction takes constant time.
 *
 * <p>For additional documentation, see <a href="http://algs4.cs.princeton.edu/31elementary">Section
 * 3.1</a> of <i>Algorithms, 4th Edition</i> by Robert Sedgewick and Kevin Wayne. For other
 * implementations, see {@link ST}, {@link BST}, {@link SequentialSearchST}, {@link RedBlackBST},
...
public class BinarySearchST<Key extends Comparable<Key>, Value> {
   ...
   *
   * @return the number of keys in this sybol table between <tt>lo</tt> (inclusive) and <tt>hi</tt>
   *     (exclusive)
  ...
  /**
   * Returns all keys in this symbol table as an <tt>Iterable</tt>. To iterate over all of the keys
  ...
  /**
   * Returns all keys in this symbol table in the given range, as an <tt>Iterable</tt>.
   *
   * @return all keys in this symbol table between <tt>lo</tt> (inclusive) and <tt>hi</tt>
   *     (exclusive)
   * @throws NullPointerException if either <tt>lo</tt> or <tt>hi</tt> is <tt>null</tt>
   */
