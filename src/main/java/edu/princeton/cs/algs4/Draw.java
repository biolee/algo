...
public final class Draw implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
   ...
   *
   * @param value the value, typically <code>JFrame.EXIT_ON_CLOSE</code> (close all windows) or
   *     <code>JFrame.DISPOSE_ON_CLOSE</code> (close current window)
   */
  ...
  /**
   * Displays on screen, pause for <tt>t</tt> milliseconds, and turn on <em>animation mode</em>.
   * Subsequent calls to drawing methods such as <tt>line()</tt>, <tt>circle()</tt>, and
   * <tt>square()</tt> will not be displayed on screen until the next call to <tt>show()</tt>. This
   * is useful for producing animations (clear the screen, draw a bunch of shapes, display on screen
   * for a fixed amount of time, and repeat). It also speeds up drawing a huge number of shapes
   * (call <tt>show(0)</tt> to defer drawing on screen, draw the shapes, and call <tt>show(0)</tt>
   * to display them all on screen at once).
   *
  ...
  /**
   * Displays on-screen and turn off animation mode. Subsequent calls to drawing methods such as
   ...
   * @return <tt>true</tt> if <tt>keycode</tt> is currently being pressed; <tt>false</tt> otherwise
   */
