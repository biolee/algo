...
public class AndroidUnlockPatterns {
  ...
  public int numberOfPatterns(int m, int n) {
    ...
    skip[1][9] =
        skip[9][1] =
            skip[2][8] = skip[8][2] = skip[3][7] = skip[7][3] = skip[6][4] = skip[4][6] = 5;
    ...
    for (int i = m; i <= n; ++i) {
      ...
      totalNumber +=
          DFS(visited, skip, 1, i - 1)
              * 4; // 1, 3, 7, and 9 are symmetric so multiply this result by 4
      totalNumber +=
  ...
  int DFS(boolean visited[], int[][] skip, int current, int remaining) {
    ...
    for (int i = 1; i <= 9; ++i) {
      ...
      // if the current node has not been visited and (two numbers are adjacent or skip number has
