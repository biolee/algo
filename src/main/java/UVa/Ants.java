...
// When
// a walking ant reaches an end of the pole, it immediately falls off it. When two ants meet they
// turn
// back and start walking in opposite directions. We know the original positions of ants on the
// pole,
// unfortunately, we do not know the directions in which the ants are walking. Your task is to
// compute
...
// for each
//     case start with two integer numbers: the length of the pole (in cm) and n, the number of ants
// residing
//     on the pole. These two numbers are followed by n integers giving the position of each ant on
// the pole
//     as the distance measured from the left end of the pole, in no particular order. All input
// integers are
//     not bigger than 1000000 and they are separated by whitespace.
//
// Output
//     For each case of input, output two numbers separated by a single space. The first number is
// the earliest
//     possible time when all ants fall off the pole (if the directions of their walks are chosen
// appropriately)
