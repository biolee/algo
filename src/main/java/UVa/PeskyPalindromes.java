...
// A palindrome is a sequence of one or more characters that reads the same from the left as it does
// from
// the right. For example, Z, TOT and MADAM are palindromes, but ADAM is not.
// Your job, should you choose to accept it, is to write a program that reads a sequence of strings
// and
// for each string determines the number of UNIQUE palindromes that are substrings.

// Input
//     The input file consists of a number of strings (one per line), of at most 80 characters each,
// starting in
//     column 1.
...
//     where input string is replaced by the actual input string and nnnn is replaced by the number
// of
